...
/*
function updateLangPosition(lang) {
  const nav = document.getElementById("mainNav");
  const langSwitcher = document.getElementById("langSwitcher");
  const navList = document.getElementById("navList");

  if (!nav || !langSwitcher || !navList) return;

  if (lang === "ar") {
    // اتجاه RTL → اللغة يسار، القوائم يمين
    nav.style.flexDirection = "row-reverse";
    navList.style.flexDirection = "row-reverse";
    langSwitcher.style.marginLeft = "0";
    langSwitcher.style.marginRight = "auto";
  } else {
    // اتجاه LTR → اللغة يمين، القوائم يسار
    nav.style.flexDirection = "row";
    navList.style.flexDirection = "row";
    langSwitcher.style.marginLeft = "auto";
    langSwitcher.style.marginRight = "0";
  }
}*/